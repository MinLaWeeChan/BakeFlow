import { formatCurrency, toMyanmarNumber } from '../utils/formatCurrency';
...
export default function SummaryCards({ stats, loading }) {
  ...
  return (
    <div className="row g-3 mb-4">
      <Card title={t('totalOrders')} value={num(stats.totalOrders)} icon="receipt" className="kpi-primary" />
      <Card title={t('totalRevenue')} value={formatCurrency(stats.totalRevenue, lang)} icon="cash-coin" className="kpi-accent" />
      <Card title={t('pendingOrders')} value={num(stats.pendingOrders)} icon="hourglass-split" className="kpi-pending" />
      <Card title={t('completedOrders')} value={num(stats.completedOrders)} icon="check2-circle" className="kpi-complete" />
    </div>
  );
}
