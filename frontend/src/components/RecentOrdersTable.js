...
export default function RecentOrdersTable({ orders, loading, error }) {
  ...
  // Translate order status values to Myanmar when lang='my'
            ...
            <table className="table table-striped table-hover align-middle">
              ...
              <tbody>
                {orders.slice(0, 10).map(order => (
                  <tr key={order.id}>
                    <td className="fw-semibold">#{lang === 'my' ? toMyanmarNumber(order.id) : order.id}</td>
                    <td>{order.customer_name}</td>
                    <td>{order.items?.[0]?.product || '—'}</td>
                    <td>{formatDate(order.created_at, lang)}</td>
                    <td><span className={`badge bg-${statusColor(order.status)} px-3 py-2`}>{translateStatus(order.status)}</span></td>
                    <td className="text-end">{formatCurrency(order.total_amount, lang)}</td>
