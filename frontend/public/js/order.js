    ...
    if (opts.cakes && opts.cakes.length > 0) {
        ...
        opts.cakes.forEach((cake, i) => {
            ...
            const parts = [
                ...
                cake.sizeExtra > 0 ? `Size +${formatCurrency(cake.sizeExtra)}` : '',
                cake.layerExtra > 0 ? `Layer +${formatCurrency(cake.layerExtra)}` : '',
                cake.creamExtra > 0 ? `Cream +${formatCurrency(cake.creamExtra)}` : '',
        ...
        const parts = [
            ...
            sizePrice > 0 ? `Size price: ${formatCurrency(sizePrice)}` : '',
            layerPrice > 0 ? `Layer price: ${formatCurrency(layerPrice)}` : '',
            creamPrice > 0 ? `Cream price: ${formatCurrency(creamPrice)}` : '',
            cakeMessage ? `Cake message: ${cakeMessage}` : '',
            notes ? `Notes: ${notes}` : '',
        ].filter(Boolean);
        const itemNote = parts.join('\n');
...
function showCustomOrderChoiceDialog(choiceData, orderData, name, phone) {
    ...
    orderInfo.textContent = `Order #BF-${existingOrderId} • ${existingOrder.items || 0} items • ${formatCurrency(Number(existingOrder.amount || 0))}`;
...
function showOrderChoiceDialog(choiceData, orderData, name, phone) {
    ...
    title.textContent = strings.activeOrders || 'Active Orders';
    title.style.cssText = `
        margin: 0 0 12px 0;
        font-size: 22px;
        font-weight: 700;
    ...
    if (!choiceData.block_new_order) {
        ...
        const lang = window.currentLang || 'en';
        const strings = window.STRINGS ? window.STRINGS[lang] : {};
        addBtn.textContent = `✨ ${strings.newOrder || 'Create New Order Instead'}`;
        addBtn.style.cssText = `
            flex: 1;
            min-width: 200px;
            padding: 14px 24px;
