...
function updateCart() {
    ...
    if (barItemsEl) {
        ...
        const displayCount = lang === 'my' ? toMy(itemCount) : itemCount;
        const itemLabel = itemCount === 1 ? (strings.item || 'item') : (strings.items || 'items');
        // Update both if possible, or just the number if barItemsDisplay not available
        barItemsEl.textContent = displayCount;
        const barItemsDisplay = document.getElementById('barItemsDisplay');
        if (barItemsDisplay) {
            barItemsDisplay.innerHTML = `<strong id="barItems">${displayCount}</strong> ${itemLabel}`;
        }
    }

    // Calculate if we should hide subtotal (if no discount)
    const subtotalRow = document.getElementById('barSubtotalRow');
    if (subtotalRow) {
        // Simple case: no promotion calculation yet
        ...
        window.calculateCheckoutWithPromotions(checkoutItems)
            .then(checkout => {
                ...
                if (useCheckout) {
                    ...
                    if (barTotalEl) barTotalEl.textContent = formatCurrency(finalTotal);
                    if (barCheckoutEl) barCheckoutEl.textContent = `${strings.checkout || 'Checkout'} • ${formatCurrency(finalTotal)}`;
                    if (barSavingsEl) {
                        barSavingsEl.style.display = 'none';
                        barSavingsEl.textContent = '';
                    }
                    if (pendingSchedule && barTotalEl) {
                        barTotalEl.textContent = formatCurrency(finalTotal);
                    }
                    return;
                }
                ...
                if (barDiscountRowEl) barDiscountRowEl.style.display = 'none';
                if (barTotalEl) barTotalEl.textContent = formatCurrency(total);
                if (barCheckoutEl) barCheckoutEl.textContent = `${strings.checkout || 'Checkout'} • ${formatCurrency(total)}`;
                if (barSavingsEl) {
                    barSavingsEl.style.display = 'none';
                    barSavingsEl.textContent = '';
                }
